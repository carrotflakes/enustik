...
export default class App extends React.Component {
    ...
    switch (this.state.mode) {
      case 'putting': {
        ...
        this.setState({
          event: {
            ...event,
            duration: Math.max(resolution, tick - event.start)
          }
        });
        break;
      }
  ...
  notePosition({x, y}) {
    ...
    return {
      ...
      tick: ((x - 25) / this.state.widthScale | 0) * resolution
    };
  }
  ...
  render() {
    const note = event => {
      return <rect x={event.start * this.state.widthScale / resolution}
                     ...
                     width={event.duration * this.state.widthScale / resolution}
                     height={this.state.heightScale}
                     key={event.id}/>;
    }
